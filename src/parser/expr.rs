...
impl<'a> ExprParser<'a> {
    ...
    pub fn parse_expr(&mut self, prev_binding_power: f32) -> R<Expr> {
        ...
        let mut left_expression = match left_token.token_data {
            ...
            TokenData::Operation(op) => {
                return Err(
                    format!("Found operation {:?} at the start of an expression", op).into(),
                );
            }
        ...
        loop {
            ...
            match &next_token.token_data {
                TokenData::Operation(op) => {
                    ...
                    left_expression = Expr::BinaryExpr(BinaryExpr::new(
                        left_expression,
                        right_expression,
                        op_type,
                    ));
    ...
    fn parse_identifier(&self, identifier: String) -> R<Expr> {
        if matches!(
            self.tokens.first_guarantee()?.token_type,
            TokenType::OpenParen
        ) {
            self.parse_function()
        }
    ...
    fn parse_variable(&self, name: String) -> R<Expr> {
        match self.variables.get_variable_by_name(&name) {
            ...
            Option::None => Err(format!("Variable {} not defined", name).into()),
        }
    }
}
